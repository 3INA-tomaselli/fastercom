...
<?php if ($classi): ?>
    ...
    <?php $idClasse = isset($_GET['classe_id']) ? (int)$_GET['classe_id'] : 0; ?>
        ...
        <select name="classe_id" id="classe_id" onchange="this.form.submit()">
            ...
            <?php foreach ($classi as $classe): ?>
                <option 
                    ...
                    <?= $idClasse == $classe['classe_id'] ? 'selected' : '' ?>
                >
                ...
                <?php if (!empty($info)): ?>
                    <?php foreach ($info as $i): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($i['studente_cognome'] ?? 'N/D'); ?></td>
                            <td><?= htmlspecialchars($i['classe_nome'] ?? 'N/D'); ?></td>
                            <td><?= htmlspecialchars($i['media_voti'] ?? 'N/D'); ?></td>
                            <td><a href="inserimentoVoto.php?id=<?= (int)$i['studente_id'] ?>">Inserisci voto</a></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
